...
import { Shield, CheckCircle, XCircle, Trash2, Check, EyeOff, Eye } from "lucide-react";
          ...
          <div className="space-y-8">
            ...
            {tab === "blog" && (
              <div className="bg-white rounded-2xl border border-border/40 shadow-sm p-6 overflow-x-auto">
                <h2 className="text-xl font-bold text-foreground mb-6">Articles du Blog</h2>
                {blogs.length === 0 ? <p className="text-sm text-muted-foreground">Aucun article de blog.</p> : null}
                {blogs.length > 0 && (
                    ...
                    <tbody>
                      {blogs.map(b => (
                        <tr key={b.id} className="border-b last:border-0 hover:bg-muted/50 transition-colors">
                          <td className="py-3 px-4 text-sm font-medium">{b.title}</td>
                          <td className="py-3 px-4 text-sm text-muted-foreground capitalize">{b.category}</td>
